...
class DynamicCropTableBody extends StatefulWidget {
  ...
  const DynamicCropTableBody({
    super.key,
    this.endpoint = 'http://127.0.0.1:8000/crop/get/1',
...
class DynamicCropTableBodyState extends State<DynamicCropTableBody> {
  ...
  Future<void> _deleteCrop(Map<String, dynamic> crop) async {
    final userId = crop['user_id'];
    final cropId = crop['id'];

    if (userId == null || cropId == null) {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(content: Text('Missing user_id or crop id')),
      );
      return;
    }
    ...
    final confirmed = await showDialog<bool>(
      ...
      builder: (context) => AlertDialog(
        ...
        content: Text('Are you sure you want to delete "${crop['name'] ?? 'this crop'}"?'),
        actions: [
          TextButton(
    ...
    try {
      final url = Uri.parse('http://127.0.0.1:8000/crop/delete/$userId/$cropId');
    ...
    if (_data.isEmpty) {
      return Center(
        child: Column(
          mainAxisSize: MainAxisSize.min,
          children: [
            const Text('No dynamic records discovered.'),
            const SizedBox(height: 16),
  ...
  List<String> _getVisibleKeys(double width) {
    final allKeys = _data.first.keys
